...
// Layout adjustments if sales rep doesn't see the right sidebar widget cleanly
const salesTableContainerStyle = {
  ...tableContainerStyle,
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Grab role configuration context to control financial metrics display parameters
  ...
  return (
    <div>
      ...
      {/* KPI Cards: Only visible for Owners */}
      {isOwner && (
      ...
      <div style={isOwner ? tableContainerStyle : salesTableContainerStyle}>
          ...
          <table style={tableStyle}>
            <thead style={tableHeadRowStyle}>
              <tr>
                ...
                {isOwner && <th style={tableHeadCellStyle}>Buy</th>}
                {isOwner && <th style={tableHeadCellStyle}>Sell</th>}
                <th style={tableHeadCellStyle}>Qty Remaining</th>
                <th style={tableHeadCellStyle}>Stock Sold</th>
                {isOwner && <th style={tableHeadCellStyle}>Profit</th>}
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id}>
                  ...
                  {isOwner && <td style={tableBodyCellStyle}>{product.buyPrice}</td>}
                  {isOwner && <td style={tableBodyCellStyle}>{product.sellPrice}</td>}
                  <td style={tableBodyCellStyle}>{product.inStock}</td>
                  <td style={tableBodyCellStyle}>{product.stockSold}</td>
                  {isOwner && <td style={tableBodyCellStyle}>{product.profit}</td>}
        ...
        {/* Top Selling Products Widget: Only visible for Owners */}
        {isOwner && (
